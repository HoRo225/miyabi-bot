...
test("schema v4 removes agent state without deleting audit history", () => {
  ...
  try {
    ...
    legacy.exec([
      ...
      ");",
      "CREATE INDEX agent_pending_actions_expiry_idx ON agent_pending_actions(status, expires_at);",
    ...
    legacy.prepare("INSERT INTO agent_pending_actions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(
      "pending", "guild", "channel", "message", "requester", "Requester", "tool", "{}", "write", "pending",
      "2026-08-02T01:00:00.000Z", null, null, "2026-08-02T00:00:00.000Z", "2026-08-02T00:00:00.000Z"
    );
    ...
    insertSetting.run("reply_mention_user", "false", "2026-08-01T00:00:00.000Z");
    ...
    legacy.prepare("INSERT INTO preserved_rows VALUES (?, ?)").run(1, "kept");
    legacy.close();
    ...
    assert.equal(store.db.prepare("SELECT name FROM sqlite_master WHERE name = ?").get("agent_pending_actions_expiry_idx"), undefined);
    ...
    assert.equal(store.setting("reply_mention_user"), "false");
    assert.equal((store.db.prepare("SELECT count(*) AS count FROM audit_logs").get() as { count: number }).count, 1);
    const preserved = store.db.prepare("SELECT id, value FROM preserved_rows").get() as { id: number; value: string };
    assert.equal(preserved.id, 1);
    assert.equal(preserved.value, "kept");
    store.close();
  } finally {
